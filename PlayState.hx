...
class PlayState extends MusicBeatState
{
	...
	// --- added for third strumline support ---
	public static var enableThirdStrumline:Bool = true; // toggle third lane
	public static var useLegacyThirdCompat:Bool = true; // remap 4..7 -> 8..11 when enabled
	...
	override public function create()
	{
		...
		// init extra strum group (third lane)
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				for (note_i in 0...notes.length) {
					var note:Note = cast notes.members[note_i];
					if (note != null && note.active) {
						// [MODIFIED for 3 players]
						var strumID:Int = note.noteData;
						if (strumID >= 0 && strumID <= 3) {
							note.mustPress = false; // opponent
							note.player = 1;
						} else if (strumID >= 4 && strumID <= 7) {
							note.mustPress = true; // player3
							note.player = 2;
						} else if (strumID >= 8 && strumID <= 11) {
							note.mustPress = true; // player1
							note.player = 0;
						}
						note.alpha *= 0.35;
					}
	...
	// --- added helper: unified strum X coordinate ---
	public function getStrumXFromIndex(i:Int):Float {
		var screenW:Float = FlxG.width;
		var spacing:Float = 112; // adjust if needed
		if (!enableThirdStrumline) {
			if (i < 4) return screenW/2 - spacing*3 + i * spacing;
			return screenW/2 + spacing * (i - 4);
		} else {
			if (i < 4) return screenW/2 - spacing*4 + i * spacing;                 // left
			else if (i < 8) return screenW/2 - spacing + (i - 4) * spacing;        // middle
			else return screenW/2 + spacing*2 + (i - 8) * spacing;                 // right
		}
	}
	// --- end helper ---
